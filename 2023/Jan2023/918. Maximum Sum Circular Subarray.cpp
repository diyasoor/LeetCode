...
public:
    int maxSubarraySumCircular(vector<int>& nums) {
        int total=0;
        int maxStraightSum=INT_MIN, tempMaxSum=0;
        int minStraightSum=INT_MAX, tempMinSum=0;
        
        for(int i=0;i<nums.size();i++) {
            total+=nums[i];
            
            tempMaxSum+=nums[i];
            maxStraightSum= max(tempMaxSum, maxStraightSum);
        ...
        // for cases when all the nums in the array are negative
        if(minStraightSum==total) return maxStraightSum;
        return max(maxStraightSum, (total-minStraightSum));    
    }
};
