...
destination of that snake or ladder is board[r][c]. Squares 1 and n2 do not have a 
snake or ladder.
...
class Solution {
public:
    vector<int> findXandY(int pos, int n) {
        int row = n-((pos - 1) / n)-1;
        int col = (pos - 1) % n;
        // if row is even and n is even or row 
        // is odd and n is odd as it is a 
        // zigzag flow, so columns will change
