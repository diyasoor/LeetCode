...
class Solution {
public:
    long long dfs(int parent, int node,long long& fuel,vector<vector<int>>& adj,int& seats) {
        int person=1;
        for(auto& child: adj[node]) {
            if(child!=parent) {
                person += dfs(node, child, fuel, adj, seats);
            }
        }
        if(node!=0) fuel+=ceil((double)person/seats);
        return person;
    }
    long long minimumFuelCost(vector<vector<int>>& roads, int seats) {
        long long fuel=0;
        int n=roads.size()+1; // roads size is n-1
