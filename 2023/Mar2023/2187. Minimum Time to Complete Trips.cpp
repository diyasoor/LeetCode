...
class Solution {
public:
    long long countTrip(long long t, vector<int>& time) {
        long long cnt=0;
        for(auto x:time) cnt+= ((long long)t/x);
        return cnt;
    }
    long long minimumTime(vector<int>& time, int totalTrips) {
        ...
        while(left < right) {
            ...
            long long tripCount = countTrip(mid, time);
            
            if(tripCount<totalTrips) left=mid+1;
            else right=mid;
        }
        return left;
    }
