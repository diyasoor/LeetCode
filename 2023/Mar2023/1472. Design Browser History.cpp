...
public:
    ...
    void visit(string url) {
        // curr page -> other searched pages
        //    |
        // visit new page
        // we will delete the other searched pages
        auto prev = it;  // curr page
        prev++;          // go one step ahead
        pages.erase(prev, pages.end());
        pages.push_back(url);
        it++;
    }
    
    string back(int steps) {
        while(it!=pages.begin() && steps--) it--;
        return *it;
    }
    
    string forward(int steps) {
        while(it!=(--pages.end()) && steps--) it++;
        return *it;
    }
};
