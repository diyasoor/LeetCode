...
ai and bi. Additionally, there is a boolean array hasApple, where hasApple[i] = true 
...
Output: 8 
Explanation: 
The figure above represents the given tree where red vertices 
...
public:
    int dfs(int curr, int parent, vector<vector<int>>& adj, vector<bool>& hasApple) {
        ...
        for(auto node: adj[curr]) {
            ...
            int nodeTime = dfs(node, curr, adj, hasApple);
            if(nodeTime > 0 || hasApple[node]) 
                time += 2 + nodeTime;
        }
        return time;
    }
